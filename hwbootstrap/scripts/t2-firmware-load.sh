...
set -euo pipefail
if [ "$(uname)" != "Linux" ]; then
    echo "This script can only run on Linux/Hollywood. Exiting."
    exit 1
fi
...
echo "Detected Hollywood"
echo "Re-mounting the EFI partition"
if [[ ${1-default} = -v ]]
then
    umount -v /dev/nvme0n1p1 || true
    mkdir -v /tmp/apple-wifi-efi || true
    mount -v /dev/nvme0n1p1 /tmp/apple-wifi-efi || true
else
    umount /dev/nvme0n1p1 2>/dev/null || true
    mkdir /tmp/apple-wifi-efi 2>/dev/null || true
    mount /dev/nvme0n1p1 /tmp/apple-wifi-efi 2>/dev/null || true
fi
mountpoint=$(findmnt -n -o TARGET /dev/nvme0n1p1)
echo "Getting Wi-Fi and Bluetooth firmware"
mkdir /tmp/apple-wifi-fw
cd /tmp/apple-wifi-fw
if [[ ${1-default} = -v ]]
then
    tar xvf $mountpoint/hollywood/t2/apple-t2-firmware.tar.gz
else
    tar xf $mountpoint/hollywood/t2/apple-t2-firmware.tar.gz 2>/dev/null
fi
echo "Setting up Wi-Fi and Bluetooth"
if [[ ${1-default} = -v ]]
then
    python3 $SCRIPTPATH /tmp/apple-wifi-fw
else
    python3 $SCRIPTPATH /tmp/apple-wifi-fw >/dev/null
fi
cd /lib/firmware
if [[ ${1-default} = -v ]]
then
    tar xvf /tmp/apple-wifi-fw/t2-firmware.tar
else
    tar xf /tmp/apple-wifi-fw/t2-firmware.tar
fi
...
echo "Cleaning up"
rm -r /tmp/apple-wifi-fw
for file in "$mountpoint/hollywood/t2/brcmfmac4364b2-pcie.txt" \
            "$mountpoint/hollywood/t2/brcmfmac4364b2-pcie.txcap_blob" \
            "$mountpoint/hollywood/t2/apple-t2-firmware.tar.gz" \
            "$mountpoint/hollywood/t2/apple-t2-firmware.sh"
do
    if [ -f "$file" ]
    then
        echo "rm $file"
