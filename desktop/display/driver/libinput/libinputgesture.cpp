...
void LibInputGesture::handlePinchBegin(libinput_event_gesture *event)
{
    ...
    QWindowSystemInterface::handleGestureEvent(
                nullptr, timestamp, nullptr, Qt::BeginNativeGesture,
                pos, pos);
#else
    QWindowSystemInterface::handleGestureEvent(
...
#if QT_VERSION >= QT_VERSION_CHECK(5, 10, 0)
    QWindowSystemInterface::handleGestureEvent(
                nullptr, timestamp, nullptr,  Qt::EndNativeGesture,
                pos, pos);
#else
    QWindowSystemInterface::handleGestureEvent(
...
void LibInputGesture::handlePinchUpdate(libinput_event_gesture *event)
{
    ...
    QWindowSystemInterface::handleGestureEventWithRealValue(
                nullptr, timestamp, nullptr,  Qt::ZoomNativeGesture,
                scale, pos, pos);
    if (angle != 0.0)
        QWindowSystemInterface::handleGestureEventWithRealValue(
                    nullptr, timestamp, nullptr, Qt::RotateNativeGesture,
                    angle, pos, pos);
#else
    QWindowSystemInterface::handleGestureEventWithRealValue(
...
#if QT_VERSION >= QT_VERSION_CHECK(5, 10, 0)
    QWindowSystemInterface::handleGestureEvent(
                nullptr, timestamp, nullptr, Qt::SwipeNativeGesture,
                pos, pos);
#else
    QWindowSystemInterface::handleGestureEvent(
