...
static void pageFlipHandler(int fd, unsigned int sequence, unsigned int tv_sec, unsigned int tv_usec, void *user_data)
{
    ...
    HWEglFSKmsEventReaderThread *t = static_cast<HWEglFSKmsEventReaderThread *>(QThread::currentThread());
    t->eventHost()->handlePageFlipCompleted(user_data);

    HWEglFSKmsScreen *screen = static_cast<HWEglFSKmsScreen *>(user_data);
...
bool HWEglFSKmsEventHost::event(QEvent *event)
{
...
void HWEglFSKmsEventHost::updateStatus()
{
...
void HWEglFSKmsEventHost::handlePageFlipCompleted(void *key)
{
    void **begin = &completedFlips[0];
    void **end = begin + MAX_FLIPS;
    void **p = begin;
...
void HWEglFSKmsEventReaderThread::run()
{
...
HWEglFSKmsEventReader::~HWEglFSKmsEventReader()
{
    destroy();
}

void HWEglFSKmsEventReader::create(HWEglFSKmsDevice *device)
{
    destroy();
    ...
    m_thread = new HWEglFSKmsEventReaderThread(m_device->fd());
    m_thread->start();
...
void HWEglFSKmsEventReader::destroy()
{
    if (!m_device)
        return;
...
void HWEglFSKmsEventReader::startWaitFlip(void *key, QMutex *mutex, QWaitCondition *cond)
{
    if (m_thread) {
