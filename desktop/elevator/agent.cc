      ...
      m_infobox(nullptr)
{
    ...
    m_registered = registerListener(session, QStringLiteral("/org/originull/Elevator/AuthenticationAgent"));
}
...
bool PolicykitAgent::registered() const
{
    return m_registered;
}
                                            ...
                                            PolkitQt1::Agent::AsyncResult *result)
{
    if (m_inProgress)
    {
        QMessageBox::information(nullptr, tr("PolicyKit Information"),
                                 tr("Another authentication is in progress. Please try again later."));
        return;
    }
    m_inProgress = true;
    deleteSessions();
