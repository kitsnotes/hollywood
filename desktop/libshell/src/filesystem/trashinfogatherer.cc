...
/*!
    Creates thread
*/
LSTrashInfoGatherer::LSTrashInfoGatherer(QObject *parent)
...
/*!
    Destroys thread
*/
LSTrashInfoGatherer::~LSTrashInfoGatherer()
{
...
QStringList LSTrashInfoGatherer::watchedFiles() const
{
...
QStringList LSTrashInfoGatherer::watchedDirectories() const
{
#if QT_CONFIG(filesystemwatcher)
    if (watcher)
...
void LSTrashInfoGatherer::watchPaths(const QStringList &paths)
{
#if QT_CONFIG(filesystemwatcher)
...
void LSTrashInfoGatherer::unwatchPaths(const QStringList &paths)
{
#if QT_CONFIG(filesystemwatcher)
    if (watcher && !paths.isEmpty())
        watcher->removePaths(paths);
...
bool LSTrashInfoGatherer::isWatching() const
{
    bool result = false;
#if QT_CONFIG(filesystemwatcher)
    QMutexLocker locker(&mutex);
...
void LSTrashInfoGatherer::setWatching(bool v)
{
    QMutexLocker locker(&mutex);
    Q_UNUSED(v)
    /* if (v != m_watching) {
...
void LSTrashInfoGatherer::setResolveSymlinks(bool enable)
{
    Q_UNUSED(enable);
#ifdef Q_OS_WIN
...
bool LSTrashInfoGatherer::resolveSymlinks() const
{
#ifdef Q_OS_WIN
...
void LSTrashInfoGatherer::setIconProvider(HWFileIconProvider *provider)
{
    m_iconProvider = provider;
}

HWFileIconProvider *LSTrashInfoGatherer::iconProvider() const
{
...
*/
void LSTrashInfoGatherer::fetchExtendedInformation(const QString &path, const QStringList &files)
{
    QMutexLocker locker(&mutex);
...
*/
void LSTrashInfoGatherer::updateFile(const QString &filePath)
{
...
/*
    ...
    \sa listed()
*/
void LSTrashInfoGatherer::clear()
{
#ifndef QT_NO_FILESYSTEMWATCHER
    QMutexLocker locker(&mutex);
...
/*
    ...
    \sa listed()
*/
void LSTrashInfoGatherer::removePath(const QString &path)
{
#ifndef QT_NO_FILESYSTEMWATCHER
    QMutexLocker locker(&mutex);
...
/*
    ...
    \sa listed()
*/
void LSTrashInfoGatherer::list(const QString &directoryPath)
{
 ...
 */
void LSTrashInfoGatherer::getFileInfos(const QString &path, const QStringList &files)
{
    // List drives
    if (path.isEmpty()) {
#ifdef QT_BUILD_INTERNAL
...
void LSTrashInfoGatherer::fetch(const QFileInfo &fileInfo, QElapsedTimer &base, bool &firstTime, QVector<QPair<QString, QFileInfo> > &updatedFiles, const QString &path) {
...
#include "moc_trashinfogatherer_p.cpp"
