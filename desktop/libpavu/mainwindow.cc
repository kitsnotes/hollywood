...
MainWindow::MainWindow():
    ...
    QSettings config("originull","hollywood");
    config.beginGroup("VolumeControl");

    showVolumeMetersCheckButton->setChecked(config.value(QStringLiteral("ShowVolumeMeters"), true).toBool());

    const QSize last_size  = config.value(QStringLiteral("WindowSize")).toSize();
    if (last_size.isValid())
        resize(last_size);

    const QVariant sinkInputTypeSelection = config.value(QStringLiteral("SinkInputType"));
    if (sinkInputTypeSelection.isValid())
        sinkInputTypeComboBox->setCurrentIndex(sinkInputTypeSelection.toInt());

    const QVariant sourceOutputTypeSelection = config.value(QStringLiteral("SourceOutputType"));
    if (sourceOutputTypeSelection.isValid())
        sourceOutputTypeComboBox->setCurrentIndex(sourceOutputTypeSelection.toInt());

    const QVariant sinkTypeSelection = config.value(QStringLiteral("SinkType"));
    if (sinkTypeSelection.isValid())
        sinkTypeComboBox->setCurrentIndex(sinkTypeSelection.toInt());

    const QVariant sourceTypeSelection = config.value(QStringLiteral("SourceType"));
...
MainWindow::~MainWindow() {
    ...
    config.setValue(QStringLiteral("SinkInputType"), sinkInputTypeComboBox->currentIndex());
    config.setValue(QStringLiteral("SourceOutputType"), sourceOutputTypeComboBox->currentIndex());
    config.setValue(QStringLiteral("SinkType"), sinkTypeComboBox->currentIndex());
    config.setValue(QStringLiteral("SourceType"), sourceTypeComboBox->currentIndex());
    config.setValue(QStringLiteral("ShowVolumeMeters"), showVolumeMetersCheckButton->isChecked());
...
void MainWindow::updateCard(const pa_card_info &info) {
    ...
    if (m_cardWidgets.count(info.index))
        w = m_cardWidgets[info.index];
    else {
        m_cardWidgets[info.index] = w = new CardWidget(this);
...
bool MainWindow::updateSink(const pa_sink_info &info) {
    ...
    if (m_sinkWidgets.count(info.index))
        w = m_sinkWidgets[info.index];
    else {
        m_sinkWidgets[info.index] = w = new SinkWidget(this);
    ...
    cw = m_cardWidgets.find(info.card);
...
void MainWindow::createMonitorStreamForSinkInput(SinkInputWidget* w, uint32_t sink_idx) {
    ...
    w->peak = createMonitorStreamForSource(m_sinkWidgets[sink_idx]->monitor_index, w->index);
}
...
void MainWindow::updateSource(const pa_source_info &info) {
    ...
    if (m_sourceWidgets.count(info.index))
        w = m_sourceWidgets[info.index];
    else {
        m_sourceWidgets[info.index] = w = new SourceWidget(this);
...
void MainWindow::updateSinkInput(const pa_sink_input_info &info) {
    ...
    if (m_sinkInputWidgets.count(info.index)) {
        w = m_sinkInputWidgets[info.index];
        if (pa_context_get_server_protocol_version(get_context()) >= 13)
            if (w->sinkIndex() != info.sink)
                createMonitorStreamForSinkInput(w, info.sink);
    } else {
        m_sinkInputWidgets[info.index] = w = new SinkInputWidget(this);
...
void MainWindow::updateSourceOutput(const pa_source_output_info &info) {
    ...
    if (m_sourceOutputWidgets.count(info.index))
        w = m_sourceOutputWidgets[info.index];
    else {
        m_sourceOutputWidgets[info.index] = w = new SourceOutputWidget(this);
...
void MainWindow::updateClient(const pa_client_info &info) {
    ...
    for (auto & sinkInputWidget : m_sinkInputWidgets) {
...
void MainWindow::updateServer(const pa_server_info &info) {
    ...
    for (auto & sinkWidget : m_sinkWidgets) {
    ...
    for (auto & sourceWidget : m_sourceWidgets) {
...
void MainWindow::updateVolumeMeter(uint32_t source_index, uint32_t sink_input_idx, double v) {
    if (sink_input_idx != PA_INVALID_INDEX) {
        ...
        if (m_sinkInputWidgets.count(sink_input_idx)) {
            w = m_sinkInputWidgets[sink_input_idx];
            w->updatePeak(v);
        }
        ...
        for (auto & sourceOutputWidget : m_sourceOutputWidgets) {
...
void MainWindow::reallyUpdateDeviceVisibility() {
    ...
    for (auto & cardWidget : m_cardWidgets) {
...
void MainWindow::setSinkWidgets(const std::map<uint32_t, SinkWidget *> &newSinkWidgets)
{
    m_sinkWidgets = newSinkWidgets;
}
