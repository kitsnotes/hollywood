...
loadTZData( RegionVector& regions, ZoneVector& zones, QTextStream& in )
{
    while ( !in.atEnd() )
    {
        ...
        QStringList timezoneParts = list.at( 2 ).split( '/', Qt::SkipEmptyParts );
...
const TimeZoneData*
ZonesModel::findtz( const QString& region, const QString& zone ) const
{
...
const TimeZoneData*
findtz( double startingDistance,
...
const TimeZoneData*
ZonesModel::findtz( const std::function< double( const TimeZoneData* ) >& distanceFunc ) const
{
    const auto* officialZone = ::findtz( 1000000.0, m_private->m_zones, distanceFunc );
    const auto* altZone
        = ::findtz( distanceFunc( officialZone ), m_private->m_altZones, distanceFunc );
    ...
    return altZone ? findtz( altZone->region(), altZone->zone() ) : officialZone;
}

const TimeZoneData*
ZonesModel::findtz( double latitude, double longitude ) const
{
    ...
    return findtz( distance );
}

QObject*
ZonesModel::lookup( double latitude, double longitude ) const
{
    const auto* p = findtz( latitude, longitude );
    if ( !p )
        p = findtz( "America", "Los_Angeles" );

    if ( !p )
        qWarning() << "No zone (not even Los Angeles) found, expect crashes.";
...
RegionalZonesModel::RegionalZonesModel(ZonesModel* source, QObject* parent)
