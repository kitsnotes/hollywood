...
static QPoint
getLocationPosition( const TimeZoneData* l )
{
    return TimeZoneImageList::getLocationPosition( l->longitude(), l->latitude() );
}


TimeZoneWidget::TimeZoneWidget( const ZonesModel* zones, QWidget* parent )
    : QWidget( parent )
...
#ifdef DEBUG_TIMEZONES
    qDebug() << "Setting location" << location->region() << location->zone() << '(' << location->country() << '@'
             << location->latitude() << 'N' << location->longitude() << 'E' << ')';
    qDebug() << Logger::SubEntry << "pixel x" << pos.x() << "pixel y" << pos.y();
#endif
...
TimeZoneWidget::mousePressEvent( QMouseEvent* event )
{
    ...
    int mX = event->pos().x();
    int mY = event->pos().y();
    auto distance = [ & ]( const TimeZoneData* zone )
    {
    ...
    const auto* closest = m_zonesData->findtz( distance );
    if ( closest )
    {
