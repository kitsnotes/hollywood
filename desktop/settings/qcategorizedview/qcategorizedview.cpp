...
void QCategorizedView::Private::rowsInserted(const QModelIndex &parent, int start, int end)
{
    ...
    for (int i = start; i <= end; ++i) {
        ...
        // TODO: FIX GHETTO HACK BY CAT
        block.items.append(Private::Item());
...
void QCategorizedView::Private::updatelastBottom(uint bottom)
{
...
QSize QCategorizedView::preferredSize()
{
    int width = size().width();
    int height = 0;

    this->slotLayoutChanged();
    qDebug() << "blocks:" << d->blocks.count();
...
uint QCategorizedView::blockCount() const
{
...
void QCategorizedView::paintEvent(QPaintEvent *event)
{
    ...
    while (it != d->blocks.constEnd()) {
        ...
        opt.features |= d->alternatingBlockColors && block.alternate ? QStyleOptionViewItem::Alternate
                                                                        : QStyleOptionViewItem::None;
        opt.state |= !d->collapsibleBlocks || !block.collapsed ? QStyle::State_Open
                                                                  : QStyle::State_None;
        const int height = d->categoryDrawer->categoryHeight(categoryIndex, opt);
        QPoint pos = d->blockPosition(it.key());
        pos.ry() -= height;
        opt.rect.setTopLeft(pos);
        opt.rect.setWidth(d->viewportWidth() + d->categoryDrawer->leftMargin() + d->categoryDrawer->rightMargin());
        opt.rect.setHeight(height + d->blockHeight(it.key()));
        opt.rect = d->mapToViewport(opt.rect);
        opt.decorationPosition = QStyleOptionViewItem::Bottom;
        if (!opt.rect.intersects(viewport()->rect())) {
            ++it;
            continue;
        }
        d->categoryDrawer->drawCategory(categoryIndex, d->proxyModel->sortRole(), opt, &p);
        ++it;
    }
    //END: draw categories
